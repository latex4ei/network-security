...
\section{Identity and Authentication}
  ...
  \subsection{Identity and Identity Theft}
    ...
    \textbf{Levels of Anonymity}
    \begin{itemize}
      \item \textbf{Identity}: actual principal is known (Public pseudonyms)
      \item \textbf{Pseudonymity}: indirectly known as the holder of a specific pseudonym (Actual principal is not disclosed) (non-public pseudonyms but known by system operators)
      \item \textbf{Anonymity}: Principal is known to be part of a defined set. The members of the set cannot be distinguished individually. (Actual principal is not disclosed.) (unlinkable pseudonyms)
    \end{itemize}
    ...
    \begin{itemize}
      \item Financial Identity Theft: using someone else credit card, using another's name and SSN to obtain goods and services (e.g. open a bank account, purchase on invoice)
      \item Criminal Identity Theft: posing as another when apprehended for a crime
      \item Identity Cloning: using another's information to assume his or her
identity in daily life
      \item Business/Commercial Identity Theft: using another's business name to obtain credit
    \end{itemize}
...
\section{Firewalls, IDS and NAT Traversal}
  ...
  \subsection{Firewalls}
    ...
    \subsubsection{Stateless Firewall}
      ...
      \underline{Pro}: application independent, good performance
    ...
    \subsubsection{Stateful Firewall}
      ...
      \underline{Pro}: more powerful

      \underline{Cons}: no state for UDP, host vs. firewall state, state explosion
    ...
    \subsubsection{Application Layer Firewall}
      ...
      \underline{Cons}: many application protocols, performance
  ...
  \subsection{Firewall detection}
    \begin{itemize}
      \item \textbf{Port scanning}: traceroute, source IP of response
      \item \textbf{TTL}: let expire TTL one hop past firewall
    \end{itemize}
  }
  ...
  \subsection{Intrustion Detection and Prevention Systems}
    ...
    \subsubsection{Classification}
      ...
      Flow: scalability (+), encryption (+), not content-based (-)

      \textbf{Point of observation:}

      Host: hard deployment (-), context information (+), one sensore per machine (+)

      Network: easy deployment (+), multiple machines (+), unknown machines(-), performance (-)
...
\section{The Domain Name System Security}
  ...
  \subsection{DNSSEC}
    ...
    \begin{itemize}
      ...
      \item \textbf{RRSet}: Resource Record Set
      \item \textbf{RRSIG}: Contains a cryptographic signature
      \item \textbf{KSK}: Key-Signing Keys 
      \item \textbf{DNSKEY}: Contains a public signing key
    ...
    \begin{itemize}
      ...
      \item Must be deployed at each step in the domain lookup
    \end{itemize}
...
\section{Availability and Denial of Service}
  ...
  \sectionbox{
    ...
    DoS: Resource \textbf{Starvation} of CPU, Storage, Network

    Attacks:
    \begin{itemize}
      \item SYN-Flood: Spam Server with TCP SYNs $\ra$ needs to keep state\\ \underline{Solution}: Choose carefully constructed initial sequence number (crypto based)
...
\section{Secure Channels: Principles, VPN, SSH}
  ...
  \subsection{Security by Layer of the TCP/IP Model}
    ...
    \subsubsection{Security at Network Layer}
      secure traffic over multiple links between end points (IPSec, OpenVPN, L2TP)
  ...
  \subsection{VPN}
    ...
    \subsubsection{Tunnel Mode (Gateway-to-Gateway)}
      ...
      \begin{itemize}
        ...
        \item Transport it as payload in new IP packet
  ...
  \subsection{SSH}
    ...
    Doesn't protect against classic TCP attacks.
...
\section{Botnets + Malware Development}
  \sectionbox{
    ...
    \textbf{Opt-in Botnet}: Voluntary botnets (e.g. Orbit Ion Cannon)
  }
...
\section{Security Ecosystem and Detection Failures}
  ...
  \subsection{Security Information Provider}
    \textbf{SIP} = Private and government organizations that efficiently monitor the primary sources of security information, validate the content found, and publish their findings as security advisories in a consistent format (NVD, CVE). (e.g. CERT, X-Force(IBM), Secunia\ldots)
    ...
    \begin{itemize}
      ...
      \item \textbf{DNS Blacklists}:
        \begin{itemize}
          ...
          \item Blacklisting(DNSBL): can be done either with IP or domain name. The DNS A record indicates if the IP address is on the list or not.
        \end{itemize}
